...
/*  WHKEY      		= Column with Warehouse retained key          */
...
quit;
 ...
 %let WHKEY = %upcase(&WHKEY); 	     *Wharehouse Retained Key;
...
proc sql noprint;
        create table tempcols as
                ...
                upcase(name) not in ("&WHKEY",'VALID_FROM_DTTM', 'VALID_TO_DTTM',"&PROCESSED_DTTM")
                except
                select name
                from dictionary.columns
...
proc sql noprint;
  select compress(put(max(&WHKEY), best32.))
     into :maxkey from &PHDIM.;
quit;

/* If "maxkey" does not have a value, then set it to zero.  */ 
%if (&maxkey eq .) %then %do;
	%let maxkey = 0;
%end;
...
data match nonmatch(keep=&WHKEY &varbus VALID_FROM_DTTM VALID_TO_DTTM);
	...
	/*Campo auxiliar para guardar retained key*/
	retain aux_rkkey &maxkey;
	...
	drop key_mch key_mch_lt_min key_mch_datas aux_rkkey;
...
proc compare base=match
 ...
 by &KNCOLS;
 id &WHKEY VALID_FROM_DTTM VALID_TO_DTTM 
				%if "&keep_date_column" ne "Y" %then %do;
				&bus_date_column.
				%end; 
				;
...
data diferent;
merge match(in=a  keep=&WHKEY &KNCOLS &colsmiss) diferent(in=b);
by &KNCOLS;
if b;
run;
...
diff_app(
		drop=uVALID_TO_DTTM uVALID_FROM_DTTM aux_rkkey 
		%if "&keep_date_column" ne "Y" %then %do;
			&bus_date_column. 
		%end;
		);
